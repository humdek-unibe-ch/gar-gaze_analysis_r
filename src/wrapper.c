...
SEXP gar_analysis_frame_create( uint32_t count )
{
    ...
    SEXP label_onset_timestamp = PROTECT( Rf_allocVector( STRSXP, count ) );
        ...
        gac_aoi_collection_analysis_result_t* analysis )
{
    ...
    for( i = 0; i < analysis->aois.count; i++ )
    {
        ...
        REAL( VECTOR_ELT( df, 8 ) )[*idx] = aoi->enter_saccade_count;
    ...
    for( i = 0; i < len; i++ )
    {
        ...
        for( j = 0; j < new_sample_count; j++ )
        {
            ...
            if( res )
            {
                ...
                res = gac_aoi_collection_analyse_saccade( &h->aoic, &saccade );
